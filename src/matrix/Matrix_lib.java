...
public class Matrix_lib {
	...
	public double getConfacter( double [][] a) {
		 double cofacter =0;
		 if(a.length == 2) {
			 cofacter = a[0][0] * a[1][1] - a[0][1] * a[1][0];
		 }else {
			 ...
			 for(int i = 0;i<a.length;i++) {
                 ...
                 cofacter += a[i][index] * Math.pow(-1,(i + 1) + (index + 1 )) * this.getConfacter(tmp); 
			 }
	...
	public double [][] getInverse(double[][] a){
		...
		double tmp[][] = new double[a.length -1][a[0].length -1];
		double determinant = this.getConfacter(a);
		for(int i = 0; i< a.length; i++) {
			...
			answer[i][j] = Math.pow(-1, i + 1 + j + 1) * this.getConfacter(tmp) / determinant;
		}
		}
	...
	public double[] getSolution(double d[][],double e[]) {
		...
		for(int h = 0;h < d.length;h++) {
			...
			solution[h] = solution[h] + r[h][l] * e[l];
		}
		}
